...
class RegistrationForm(FlaskForm):
    ...
    position=StringField('Salutation',validators=[DataRequired()])
    teamId=IntegerField('Phone number',validators=[DataRequired()])
    teamName=StringField('City',validators=[DataRequired()])
...
class UserChoiceIterable(object):
    def __iter__(self):
        ...
        choices=[(user.id,f'{user.fullname}, city {Team.query.filter_by(id=user.teamId).first().teamName}') for user in users] 
...
class BookmeetingForm(FlaskForm):
    title=StringField('Purpose of Visit',validators=[DataRequired()])
    ...
    duration=SelectField('Choose duration of stay(in days)',coerce=int,choices=[(i,i) for i in range(1,6)])
    participants_user=SelectMultipleField('Which friends are you staying with?',coerce=int,choices=UserChoiceIterable(),option_widget=widgets.CheckboxInput(),widget=widgets.ListWidget(prefix_label=False),validators=[DataRequired()])
    participants_partner=SelectMultipleField(' ',coerce=int,option_widget=widgets.CheckboxInput(),widget=widgets.ListWidget(prefix_label=False))
...
class CancelbookingForm(FlaskForm):
    ...
    ids=SelectField('Choose booking to cancel',coerce=int,choices=MeetingChoiceIterable()) 
